...
* Git hooks that run the code quality checks automatically, enabled with `git config core.hooksPath` against the version-controlled `.githooks/` directory (rather than copying into `.git/hooks/`), so they're reviewable and travel with the repo. The `pre-commit` hook runs `just format` and the `pre-push` hook runs `just lint`. New `just install_hooks` recipe installs them and `just init` runs it on first setup. The `pre-commit` hook re-stages the files you'd already staged so the fixes are committed, but aborts if a file is only partially staged (`git add -p`) to avoid sneaking unstaged hunks into the commit. Bypass any hook with `--no-verify`.
* `just format` and the source-only lint checks (`lint_py`, `lint_html`, `lint_js`) now run their tools — ruff, djLint, oxfmt, and oxlint — **natively via `uvx`/`bunx` instead of Docker**, dropping a `just format` from ~2.4s (or ~9.5s cold) to ~1s and removing the Docker round-trip from commits. Exact tool versions are read from `uv.lock` (ruff/djLint) and `package.json` (oxfmt/oxlint) at runtime via just variables, so native runs match Docker + CI with no hand-managed pins. `ty` and the `makemigrations --check` migration check still run over Docker (they need the project's installed deps); the `pre-push` hook starts the `web` service (`docker compose up --wait -d web`) if it isn't already running. A shared `_require_native_tools` recipe checks that `uv`/`bun` are on PATH and prints install guidance if not — both are now prerequisites for `just format`/`just lint`. CI is unaffected (it already runs these recipes natively via empty `*_CMD_PREFIX` overrides).
