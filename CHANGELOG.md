...
* Git hooks that run the code quality checks automatically, enabled with `git config core.hooksPath` against the version-controlled `.githooks/` directory (rather than copying into `.git/hooks/`), so they're reviewable and travel with the repo. The `pre-commit` hook runs `just format_native`, which formats the tree with ruff, djLint, oxfmt, and oxlint run natively via `uvx`/`bunx` — no Docker, so commits stay ~1s — and reads the exact tool versions from `uv.lock`/`package.json` at runtime so they match Docker + CI. It re-stages the files you'd already staged so the fixes are committed, but aborts if a file is only partially staged (`git add -p`) to avoid sneaking unstaged hunks into the commit. The `pre-push` hook runs `just lint` over Docker (its `ty` and missing-migration checks need the project's installed deps), starting the `web` service with `docker compose up --wait -d web` if it isn't already running. New `just install_hooks` recipe installs the hooks and `just init` runs it; the new `just format_native` recipe is also usable on its own. `uv` + `bun` must be on your PATH for the pre-commit hook. Bypass any hook with `--no-verify`.
