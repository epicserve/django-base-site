...
<template>
  <ToastProvider :duration="4000" swipe-direction="right">
    <ToastRoot
      ...
      :class="toast.type === 'error' ? 'border-red-200 dark:border-red-900/50' : 'border-gray-200 dark:border-gray-700'"
      @animationend="
        (e) => {
          if (!toast.open && e.animationName.includes('slide-out')) removeToast(toast.id);
        }
      "
    >
      <svg
      ...
      >
        <path
          fill-rule="evenodd"
          :d="
            'M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0' +
      ...
      <svg v-else class="mt-0.5 h-5 w-5 shrink-0 text-green-500" viewBox="0 0 20 20" fill="currentColor">
        <path
          fill-rule="evenodd"
          :d="
