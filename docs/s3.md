# Object Storage (AWS S3 + Silo)

Media uploads (avatars, future user-generated content) go through django-storages with an S3-compatible backend. Production runs against AWS S3 (or another S3-compatible service); local development runs against [Silo](https://github.com/pgsty/silo) in Docker as the `silo` compose service. Silo is a maintained, drop-in fork of the MinIO server (MinIO archived its community edition and removed its Docker Hub images in 2026). The same backend class is used in both environments.

There are two storage classes in `apps/base/storage.py`:

| Backend                                 | When to use                                                                                                                          |
|-----------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| `apps.base.storage.S3MediaStorage`      | **Default for both local dev and production.** Adds an optional URL-rewrite step for the Docker-internal vs. browser endpoint split local Docker development needs. |
...
Set the following in your production environment (don't set `MEDIA_S3_URL_ENDPOINT_URL` — that's only for the local Silo endpoint split):
...
## Local Development: Silo

Nothing to configure — `compose.yml` runs Silo as the `silo` service and `just create_env` writes sensible defaults to `.env`:

```bash
DEFAULT_FILE_STORAGE=apps.base.storage.S3MediaStorage
MEDIA_S3_ACCESS_KEY=siloadmin
MEDIA_S3_SECRET_KEY=siloadmin
MEDIA_S3_ENDPOINT_URL=http://silo:9000
...
- `MEDIA_S3_ENDPOINT_URL` — what Django uses to *talk* to Silo. Inside the Docker network, Silo is reachable at `http://silo:9000`.
...
The `web` container runs `./manage.py ensure_s3_bucket` on startup, which creates the bucket if it doesn't exist. The Silo web console is at http://localhost:9001 (creds default to `siloadmin` / `siloadmin`, the same values as `MEDIA_S3_ACCESS_KEY` / `MEDIA_S3_SECRET_KEY`, because the compose file feeds those to Silo as its root user and password).
