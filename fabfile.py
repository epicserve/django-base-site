...
from fabric.api import cd, env, execute, local, roles, run, settings, sudo, task
...
env.cron_config_files = ("example_task",)
...
def update_requirements():
    ...
    sudo(
        "%s/bin/pip install -r %s/config/requirements/base.txt -r %s/config/requirements/production.txt"
        % (env.virtualenv, env.code_dir, env.code_dir),
        user="deploy",
    )
...
def migrate(app=None):
    ...
    if app:
        sudo(
            "source %s/bin/activate; django-admin.py migrate %s --settings=%s"
            % (env.virtualenv, app, env.django_settings_module),
            user="deploy",
        )
    else:
        sudo(
            "source %s/bin/activate; django-admin.py migrate --settings=%s"
            % (env.virtualenv, env.django_settings_module),
            user="deploy",
        )
